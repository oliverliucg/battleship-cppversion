...
class BattleShipBoard
    : public Board<T>,
      public std::enable_shared_from_this<BattleShipBoard<T>> {
private:
  const int width;
  const int height;
...
public:
  ...
  BattleShipBoard(int w, int h,
                  std::unique_ptr<PlacementRuleChecker<T>> placementChecker,
                  T missInfo);
...
string BattleShipBoard<T>::removeShipFromBoardIfExist(Coordinate where) {
  ...
  return "No Ship At (" + std::to_string(where.getRow()) + "," +
         std::to_string(where.getColumn()) + ")";
}

template <typename T>
BattleShipBoard<T>::BattleShipBoard(
    int w, int h, unique_ptr<PlacementRuleChecker<T>> _placementRuleChecker,
    T _missInfo)
    : width(w), height(h),
      placementRuleChecker(std::move(_placementRuleChecker)),
      missInfo(_missInfo) {
  if (w <= 0) {
    throw MyException("BattleShipBoard's width must be positive but is " +
                      std::to_string(w));
  }
  if (h <= 0) {
    throw MyException("BattleShipBoard's height must be positive but is " +
                      std::to_string(h));
  }
}
template <typename T>
BattleShipBoard<T>::BattleShipBoard(int w, int h, T _missInfo)
    : BattleShipBoard(
          w, h,
          std::make_unique<NotNullPointerRuleChecker<T>>(
              std::make_unique<NoCollisionRuleChecker<T>>(
                  std::make_unique<InBoundRuleChecker<T>>(nullptr))),
          _missInfo) {}
// template<typename T>
// BattleShipBoard<T>::BattleShipBoard(const BattleShipBoard<T> &other):
// width(other.width), height(other.height),
// placementRuleChecker(std::make_unique<NotNullPointerRuleChecker<T>
// >(std::make_unique<NoCollisionRuleChecker<T>
// >(std::make_unique<InBoundRuleChecker<T> >(nullptr)))),
// missInfo(other.missInfo) {
//}
//
// template<typename T>
// BattleShipBoard<T> &BattleShipBoard<T>::operator=(const BattleShipBoard<T>
// &other) {
//    if(this != other){
//        this->width = other.width;
//        this->height = other.height;
//        this->placementRuleChecker =
//        std::make_unique<NotNullPointerRuleChecker<T>
//        >(std::make_unique<NoCollisionRuleChecker<T>
//        >(std::make_unique<InBoundRuleChecker<T> >(nullptr))); this->missInfo
//        = other.missInfo;
//    }
//    return (*this);
//}
template <typename T> int BattleShipBoard<T>::getWidth() { return width; }

template <typename T> int BattleShipBoard<T>::getHeight() { return height; }

template <typename T>
string BattleShipBoard<T>::tryAddShip(shared_ptr<Ship<T>> toAdd) {
  string placementProblem = placementRuleChecker->checkPlacement(
      toAdd,
...
template <typename T> bool BattleShipBoard<T>::move(Coordinate c, Placement p) {
...
template <typename T> Coordinate BattleShipBoard<T>::pickSunkShip() {
  Coordinate res(-1, -1);
  for (std::shared_ptr<Ship<T>> ship : myShips) {
    if (ship->isSunk()) {
...
template <typename T> Coordinate BattleShipBoard<T>::randomlyPickShip() {
  Coordinate res(-1, -1);
  for (std::shared_ptr<Ship<T>> ship : myShips) {
    if (ship->isSunk()) {
      if (res == Coordinate(-1, -1) || randomlyPickFrom(0, 1) == 1) {
...
template <typename T>
vector<Placement>
BattleShipBoard<T>::getAvailablePlacements(bool isToGetAll, char type_of_ship) {
...
template <typename T>
Placement
BattleShipBoard<T>::pickPlacement(vector<Placement> availablePlacements) {
...
template <typename T> int BattleShipBoard<T>::getNumOfSunkShips() {
  int num = 0;
  for (std::shared_ptr<Ship<T>> ship : myShips) {
    if (ship->isSunk()) {
      ++num;
...
template <typename T> int BattleShipBoard<T>::numOfShips() {
  return myShips.size();
}
template <typename T> bool BattleShipBoard<T>::isInRange(Coordinate where) {
  return where.getRow() >= 0 && where.getRow() < height &&
         where.getColumn() >= 0 && where.getColumn() < width;
}
