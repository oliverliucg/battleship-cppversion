...
ShipFactory::createRectangleShip(Placement p, int w, int h, char letter) {
...
std::shared_ptr<Ship<char>> ShipFactory::createBattleShip(Placement p,
  ...
  return std::make_shared<BattleShip<char>>(p.getWhere(), p.getOrientation(),
                                            letter, '*');
}
...
std::shared_ptr<Ship<char>> ShipFactory::createCarrier(Placement p,
  ...
  return std::make_shared<Carrier<char>>(p.getWhere(), p.getOrientation(),
                                         letter, '*');
}
