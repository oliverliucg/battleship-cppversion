...
public:
    InBoundRuleChecker(unique_ptr<PlacementRuleChecker<T> > _next):PlacementRuleChecker<T>(std::move(_next)){}
};

template<typename T>
string InBoundRuleChecker<T>::checkMyRule(std::shared_ptr<Ship<T> > theShip, std::shared_ptr<Board<T> > theBoard){
